...
class TestCommandHandlerPluginDepartures:
    """Tests for plugin departures command"""
    ...
    def handler(self, mock_ibapi):
        ...
        mock_pe = MagicMock()
        mock_pe.get_departures.return_value = {}
        handler.plugin_executive = mock_pe
        return handler

    def test_plugin_departures_empty(self, handler):
        """Returns 0 departures when board is empty"""
        from command_server import CommandStatus

        result = handler.handle_plugin(["departures"])

        assert result.status == CommandStatus.SUCCESS
        assert "0 departure(s)" in result.message
        assert result.data["departures"] == {}

    def test_plugin_departures_after_unload(self, handler):
        """Departures shows entry after unload"""
        from command_server import CommandStatus

        handler.plugin_executive.get_departures.return_value = {
            "test_plugin:abcd1234": {
                "plugin_name": "test_plugin",
                "instance_id": "abcd1234-full-id",
                "message": "Goodbye from test_plugin!",
                "unloaded_at": 1700000000.0,
            }
        }
        ...
        assert result.status == CommandStatus.SUCCESS
        assert "1 departure(s)" in result.message
        assert "test_plugin:abcd1234" in result.data["departures"]

    def test_plugin_departures_clear(self, handler):
        """--clear flag passes clear=True and message reflects it"""
        from command_server import CommandStatus
        ...
        result = handler.handle_plugin(["departures", "--clear"])

        handler.plugin_executive.get_departures.assert_called_with(clear=True)
        assert result.status == CommandStatus.SUCCESS
