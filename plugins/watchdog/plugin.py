...
# Known-noisy window, never eligible for escalation regardless
...
class WatchdogPlugin(PluginBase):
    ...
    def __init__(self, base_path=None, portfolio=None,
        ...
        #      didn't work.
        # There is deliberately no second tier. Killing and relaunching
        # the TWS process was tried and removed on 2026-08-14: TWS comes
        # back up asking for credentials, so a relaunch cannot restore an
        # unattended system — it only replaces a running-but-degraded TWS
        # with a login prompt, and each attempt leaves another prompt
        # behind. The code already knew ("Daily auto-restart is not
        # enabled", observed live 2026-07-20) and kept the tier anyway.
        # When reconnect is not enough, the alert reaching a human IS the
        # recovery path.
        #
        # The timeout pair is "timeout, not keep-out" (deliberately looser
        # off-hours, not disabled): while any plugin's declared
        # trading_hours() window is active, escalate reasonably fast;
        # outside all of them, still escalate eventually rather than leave
        # a real fault to fester unnoticed until the next session.
        ...
        # After a reconnect: don't evaluate escalation again until warmup
        # elapses. Cooldown is the separate, harder floor on repeating the
        # action, checked independently so it holds even if warmup has
        # elapsed but feeds are stale again.
        ...
        # never escalate inside these, regardless
    ...
    def start(self) -> bool:
        ...
        if saved:
            ...
            # a reconnect just happened and immediately re-escalate
    ...
    # =========================================================================
    # ESCALATION — force a reconnect when resubscription alone isn't
    # clearing the fault (the TWS process is never touched
    ...
    def _check_stale_feed_escalation(self) -> None:
        ...
        doesn't clear it. There is no further tier (see the
        ...
        """
        if not self.auto_reconnect_on_stale:
            return
        if not self.portfolio or not hasattr(self.portfolio, "keep_up_to_date_feeds"):
            return
        if not getattr(self.portfolio, "connected", True):
        ...
        # first clean check on 2026-07-20; a plain
        # reconnect should clear faster, hence its own shorter warmup).
        warming_until = self._last_reconnect + self.reconnect_warmup_seconds
    ...
    def _in_blackout(self, now: Optional[datetime] = None) -> bool:
        ...
        blackout window — never eligible for reconnect regardless of
    ...
    def _force_reconnect(self, reason: str) -> None:
        ...
        reconnect, which never touches the TWS process."""
